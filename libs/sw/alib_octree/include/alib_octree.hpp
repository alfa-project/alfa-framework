...
struct AlfaBB {
  ...
  float max_y;
  float max_z;
  AlfaBB(float min_x = 0, float min_y = 0, float min_z = 0, float max_x = 0,
         float max_y = 0, float max_z = 0)
      : min_x(min_x),
        min_y(min_y),
        min_z(min_z),
        max_x(max_x),
...
class AlfaOctree {
 ...
 private:
  ...
  void insert_pointcloud_multi_thread_vector_handler(
      std::vector<AlfaPoint> pointcloud, int childIndex);
  void insert_pointcloud_multi_thread_handler(
      pcl::PointCloud<AlfaPoint>::Ptr pointcloud, int childIndex);
  void init_node_from_occupation_code_DFS(AlfaOctreeNode *node,
                                          std::vector<unsigned char> &code,
                                          unsigned int *index);

  void insert_pointcloud_multi_thread(
      pcl::PointCloud<AlfaPoint>::Ptr pointcloud);

  void insert_pointcloud_single_thread(
      pcl::PointCloud<AlfaPoint>::Ptr pointcloud);
  ...
  void retrieve_occupation_code_DFS(AlfaOctreeNode *node,
                                    std::vector<unsigned char> &code);
