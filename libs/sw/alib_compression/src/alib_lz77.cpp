  ...
  while (index < input_size) {
    ...
    for (size_t j = search_start; j < index; ++j) {
      ...
      if (match_length > best_length) {
        ...
        best_offset = index - j;
        next_char = (index + match_length < input_size)
    ...
    encoded.push_back(
        static_cast<unsigned char>((best_offset >> 8) & 0xFF));  // Offset MSB
    encoded.push_back(
        static_cast<unsigned char>(best_offset & 0xFF));  // Offset LSB
    encoded.push_back(
        static_cast<unsigned char>(best_length & 0xFF));  // Length
