...
#define DEFAULT_TOPIC "/ext_pcl_octree_compression_encoder_pointcloud"
...
void handler(AlfaNode *node) {
  ...
  pcl::io::OctreePointCloudCompression<pcl::PointXYZ> pointcloud_decompressor(
  ...
  // Check if the decompressed point cloud is not empty and fill the output
  if (!decompressed_cloud->empty()) {
    for (const auto &pcl_point : decompressed_cloud->points) {
