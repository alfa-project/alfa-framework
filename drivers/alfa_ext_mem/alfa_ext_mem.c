...
static int alfa_ext_open(struct inode *inode, struct file *filp) {
  if (file_open) {
    pr_err(
...
static int alfa_ext_release(struct inode *inode, struct file *filp) {
  if (!file_open) {
    pr_err(
...
static int alfa_ext_mmap(struct file *filp, struct vm_area_struct *vma) {
...
static const struct file_operations alfa_ext_fops = {
    .owner = THIS_MODULE,
    .open = alfa_ext_open,
...
static int alfa_ext_probe(struct platform_device *pdev) {
  ...
  ret = register_chrdev(0, DEVICE_NAME, &alfa_ext_fops);
  if (ret < 0) {
