...
PCLViewer::PCLViewer(int argc, char **argv, QWidget *parent) : QMainWindow(parent),
                                                               ui(new Ui::PCLViewer),
                                                               filtering_axis_(1), // = y
                                                               color_mode_(4)      // = Rainbow
{
  ...
  viewer_input.reset(new pcl::visualization::PCLVisualizer(renderer_input, render_input_window, "viewer", false));
  viewer_output.reset(new pcl::visualization::PCLVisualizer(renderer_output, render_output_window,"viewer", false));
  ...
  viewer_output->setupInteractor(ui->qvtkWidget_alfa->interactor(), ui->qvtkWidget_alfa->renderWindow());
  ...
  viewer_input->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "cloud");
  viewer_output->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "cloud");
...
void PCLViewer::mouseEventClick(const pcl::visualization::PointPickingEvent &event, void *viewer_inputvoid)
{
...
void PCLViewer::saveScreenButtonPressed()
{
  ...
  QString filename = QFileDialog::getSaveFileName(this, tr("Image Data"), "/home", tr("PNG (*.png *.jpeg)"));
...
void PCLViewer::colorCloudDistances(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud,pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_int)
{
  ...
  for (PointCloud<PointXYZRGBA>::iterator cloud_it = cloud->begin(); cloud_it != cloud->end(); ++cloud_it)
  {
    switch (filtering_axis_)
    {
    case 1: // y
    ...
    default: // z
      ...
      for (PointCloud<PointXYZI>::iterator cloud_it = cloud_int->begin(); cloud_it != cloud_int->end(); ++cloud_it)
      {

            if (min > cloud_it->intensity)
              min = cloud_it->intensity;

            if (max < cloud_it->intensity)
              max = cloud_it->intensity;
  ...
  for (PointCloud<PointXYZRGBA>::iterator cloud_it = cloud->begin(); cloud_it != cloud->end(); ++cloud_it)
  {
    ...
    {
    case 0:                                                 // x
      value = std::lround(((*cloud_int)[pointer].intensity*-min)*lut_scale); // Round the number to the closest integer
      break;
    case 1: // y
      value = std::lround((cloud_it->y - min) * lut_scale);
      break;
    default: //Distance
      value = std::lround(sqrt((pow(cloud_it->x, 2) + pow(cloud_it->y, 2)+pow(cloud_it->z+10, 2)))*300/(max_len/1.1));
      break;
    }
...
void PCLViewer::buildbox()
{
  ...
  if (!ui->cbHide->isChecked())
  {
    ...
    {
      viewer_input->addCube(boxlist[i].pStart.x, boxlist[i].pEnd.x, boxlist[i].pStart.y, boxlist[i].pEnd.y, boxlist[i].pStart.z, boxlist[i].pEnd.z, 1.0, 1.0, 1.0, boxlist[i].name, 0);
    }
    ui->qvtkWidget->renderWindow()->Render();
  }
}
  ...
  if (!ui->cbHide->isChecked())
  {
    for (int i = 0; i < boxlist.size(); i++)
    {
      if (index != i)
        ...
        viewer_input->addCube(boxlist[i].pStart.x, boxlist[i].pEnd.x, boxlist[i].pStart.y, boxlist[i].pEnd.y, boxlist[i].pStart.z, boxlist[i].pEnd.z, 1.0, 0, 0, boxlist[i].name, 0);
    }
    ui->qvtkWidget->renderWindow()->Render();
  }
}
...
void PCLViewer::calculate_insidePoints(int index)
{
  ...
  for (auto &point : *raw_cloud)
  {
    if (insidepoint(boxlist[index], point))
      count++;
  }
...
bool PCLViewer::insidepoint(box mBoxl, PointXYZI p1)
{
  if (p1.x >= mBoxl.pStart.x && p1.x <= mBoxl.pEnd.x)
  {
    ...
    {
      if (p1.z >= mBoxl.pStart.z && p1.z <= mBoxl.pEnd.z)
        return true;
    }
  }
  return false;
...
void PCLViewer::on_btnSaveBox_clicked()
{
  saveBoxToFile();
}
...
void PCLViewer::saveBoxToFile()
{
  ...
  for (int i = 0; i < boxlist.size(); i++)
  {
    MyFile << boxlist[i].pStart.x << ";" << boxlist[i].pEnd.x << ";" << boxlist[i].pStart.y << ";" << boxlist[i].pEnd.y << ";" << boxlist[i].pStart.z << ";" << boxlist[i].pEnd.z << ";" << boxlist[i].index << ";" << boxlist[i].name << ";" << boxlist[i].type << "\n";
  }
}
...
{
  string myText = "";
  QString filename = QFileDialog::getOpenFileName(this, tr("Box File"), "/home/", tr("BoxFile (*.bxf);;All Files (*)"));
...
void PCLViewer::on_listWidget_itemEntered(QListWidgetItem *item)
{
  buildbox();
}
...
void PCLViewer::do_deletePointBox(pcl::PointCloud<pcl::PointXYZI>::Ptr output)
{
  ...
  for (int i = 0; i < boxlist.size(); i++)
  {
    if (boxlist[i].type == 1)
    {
      ...
      range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("z", pcl::ComparisonOps::GT, boxlist[i].pStart.z)));
      range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("z", pcl::ComparisonOps::LT, boxlist[i].pEnd.z)));
      range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("x", pcl::ComparisonOps::GT, boxlist[i].pStart.x)));
      range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("x", pcl::ComparisonOps::LT, boxlist[i].pEnd.x)));
      range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("y", pcl::ComparisonOps::GT, boxlist[i].pStart.y)));
      range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(new pcl::FieldComparison<pcl::PointXYZI>("y", pcl::ComparisonOps::LT, boxlist[i].pEnd.y)));
...
void PCLViewer::t_calback_thread()
{
    ...
    {
        mutex.lock();
        double raw_fps = (ros_interface->raw_clouds/(TIMER_INTERVAL/1000.0))*(1000/TIMER_INTERVAL);
        double alfa_fps = (ros_interface->alfa_clouds/(TIMER_INTERVAL/1000.0))*(1000/TIMER_INTERVAL);
...
void PCLViewer::set_style()
{

    this->setStyleSheet(combinear);
...
bool PCLViewer::load_topics()
{
  ...
  for(auto const& [name, type] : current_topics)
  {
    if(QString::fromStdString(type[0]).contains("PointCloud2"))
    {
        ...
        if(name == ros_interface->raw_topic.toStdString())
          raw_current_index = index;
          
        if(name == ros_interface->alfa_topic.toStdString())
          alfa_current_index = index;
  ...
  {
    ui->cb_RAW->addItem(QString::fromStdString(topic));// + "--------->" + QString::fromStdString(std::string(payload[i][1])));
...
void PCLViewer::on_load_camera_clicked()
{
    ...
    viewer_input->getCameras(cameras);
    viewer_output->setCameraParameters(cameras[0]);
    QString filename = QFileDialog::getOpenFileName(this, tr("Cam File"), "~/", tr("CamFile (*.cam);;All Files (*)"));
...
{
    QToolTip::showText(ui->pb_PointParametersHint->mapToGlobal(QPoint()), "Shift+Left Click to obtain a point's parameters");
}
...
{
  QString filename = QFileDialog::getOpenFileName(this, tr("Open point cloud"), "", tr("ROS Bag(*.db3)"));
  if (filename.isEmpty())
    return;
...
void PCLViewer::on_pb_NextFrame_clicked()
{
    ros_interface->publish_next_bag_pointcloud();
}
...
void PCLViewer::on_l_ExecOrder_currentItemChanged(QListWidgetItem *current, QListWidgetItem *previous)
{
    ...
    if(ui->l_ExecOrder->item(0)->text().compare("Noise injection"))
    {
        if(ui->cb_RemovePoints->isChecked())
        {
...
void PCLViewer::on_lw_confignodes_itemDoubleClicked(QListWidgetItem *item)
{
  ...
  newConfigWindow = new ConfigWindow(alfa_nodes[ui->lw_confignodes->currentRow()], ros_interface, this);
...
{
    MetricWindow *newMetricWindow = new MetricWindow(metric_topics[ui->lw_metrics->currentRow()], ros_interface, this);
